...
* 입출력을 제어 할 수 있는 방법    
    입출력을 제어 할 수 있는 방법은 두가지가 존재하며 메모리 맵 I/O , I/O 맵방식이 있다    
    메모리 맵 I/O는 입출력 장치ㄹ들이 사영하는 메모리가 따로 정해져 있지 않아서 장치가 사용할 메모리를 개발자가 직접 지정해야한다.    
    맵 I/O는 입출력 장치들이 사용하는 메모리가 미리 정해져 있어서 지정된 메모리르 사용하는 방법이다    
    ARM계열 프로세서들은 메모리 맵 I/O를 사용하고 PXA255와 같은 인텔 계열 프로세서는 I/O 맵 를 사용한다
* 소프트웨어에서 하드웨어는 메모리로 인식 된다.    
    프로그램은 주변기기로부터 값을 읽거나 프로그램에서 처리한 결과를 주변기기로 출력한다. 이러한 입츌력을 중간에서 연결 지어주는 것이 입출력 포트이다    
    프로그램에서는 입출력포트에서 사용할 메모리를 지정하고 지정된 메모리에 값을 읽거나 쓰면된다    
    * I/O포트 중에서 레지스터 정보   
        CON , DAT 가 있는데 COn레지스터는 포트를 사용하기 위한 설정 값들이 저장되는 레지스터   
        DAT레지스터는 입출력 장치로부터 데이터를 읽고 쓸 수 있는 레지스터 이다.    
        GPE 포트르 입력 출력 특수용도 모드로 설정할 수 있다   
        GPEDAT 레지스터는 데이터가 들어갈 메모리이다.    
        이러한 레지스터를 사용하려면 메모리를 선언해야하고 하드웨어에 접근하려면 하드웨어에서 사용하는 메모리를 선언해서 메모리에 접근할 수 있는 방법을 만들어 준다    
        예) PA[7] >> PA0 >> LED1    
        예) PA[6] >> PA1 >> LED2    
        예) PA[5] >> PA2 >> LED3    
        PA는 출력 전용으로 이루져있다.PA를 출력 전용으로 만든 이유는 LED와 같이 값을 쓰기만 하고 읽기가 필요하지 않은 장치를 위해 사용하는 포트 이기 때문이다.
* 하드웨어 접근을 위해서 포인터를 선언한다
    ```
    char *p = (char *)0x30000000;
    데이터 타입이 char이므로 문자를 읽거나 저장하기 위한 메모리 일까??
    그렇지는 않다. 임베디드에서 char는 1바이트 처리를 위한 데이터 타입이다.
    다시 정리하면 메모리 3000_0000번지부터 1 바이트를 할당 받아서 지정된 포트를 사용할 수 있으며 이 포트에는 LED가 연결될 수 있다!! 
    ```
* 하드웨어 제어 하는 방법   
    하드웨어를 제어하는 방법은 사용할 포트를 어떤 모드로 사용할지 설정하고 프로그램에서 데이터 레지스터에 값을 쓰거나 읽어 오면 된다   
    ```
    char *p = (char *)0x30000000;
    *p = 0;
    *p |= 0x1 << 6 
    << 시프트 연산자
    |= OR연산자
    PA가 사용하는 모든 메모리르 0으로 초기화 하면 안되고  LED가 사용할 수 있는 메모리만 초기화 해야한다
    LED가 사용하는 PA[7:5]비트만 사용 해야 하고 다른 장치가 사용하느 비트까지 값을  변경해서는 안된다.
    다른 비트를 수정하게 되면 다른장치에서 오류가 날 수 있다
    결론적으로는 레지스터르 다를때에는 조심할점은 다른 비트의 값은 변경하지 않고 해당 비트만 수정해야 한다는 것이다
    다른 비트는 다른 포트에서 사용하는 메모리이므로 전체를 0으로 클리어 한다면 다른 포트를 위한 설정값이나 데이터들이 모두 삭제된다
    특정 비트만 수정해야한다!!
    ``` 
*     
